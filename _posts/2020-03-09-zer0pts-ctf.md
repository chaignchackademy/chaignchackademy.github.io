...
* Injecter l'object pickle stoqué dans les cookies de session pour executer du code python
...
{% endhighlight %}
Ce cookie est serialisé, salé, chiffré, signé puis encodé par Flask, il n'est donc théoriquement pas possible pour un utilisateur de créer un cookie valide sans connaitre la clef de chiffrement secrete utilisé par Flask. Forte heureusement nous avons avec la SSTI trouvé la clef de chiffrement utilisée ```SECRET_KEY```.
...
Ci-dessous un exemple d'utilisation de cette fonction session_cookie_encoder pour créer un cookie de session connant une note avec le titre "ChaigncHackademy".
...
# Cette structure de données est serialisée avec pickle, puis encodée en base64
...
## 3) Serializer des objects python avec pickle
Pickle fait partie des libraries standards de python, cette librarie permet de serializer et déserialiser des objets python sous forme binaire.

{% highlight python %}
{% raw %}
>>> import pickle
>>> # La fonction dumps va serialiser en binaire l'object python qu'on lui donne en paramètre, en python tout est objet donc nous pouvons tout serialiser.
...
>>> # La fonction loads va déserialiser une suite d'octets (bytes) pour recréer les objets python qui y sont stoqués
...
En effet si un utilisateur controle les octets déserialisés alors il peut instancier les objets python qu'il désire et même executer des fonctions python sans notre permission. 
...
Petite subtilité, le payload utilisé par pickle peut utiliser n'importe quelle librairie installée même s'elle n'est pas explicitement importée... :)
{% highlight python %}
{% raw %}
...
## 4) Reverse shell
Maintenant il suffit d'envoyer un cookie forger avec un objet pickle délivrant nos commands pour prendre le controle du serveur, cependant ce serveur tourne dans un docker assez light, il n'y a pas ping, pas netcat ... Par contre il y a python ce que nous pouvons utiliser pour faire un reverse shell.
...
Ce payload nous donne un shell qui nous permets de sortir le flag secret afin de valider le challenge.

## 5) Socket Reuse

Un reverse shell était suffisant pour ce challenge, mais imaginons qu'un mecanisme réseau log les connexions sortantes et que nous voulons éviter d'être detecté. Une option est de réutiliser la connexion TCP déjà établie pour la requête HTTP, en effet cette socket n'est pas encore fermée.
...
Ceci était le premier writeupn, beaucoup de conceptes ont été rapidement survolés nous les aborderons en détail dans les prochains articles.
